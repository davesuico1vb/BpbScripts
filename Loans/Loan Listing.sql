...
select
  ...
  ------------------------------------------------------------------------------------------------------------								
	...
	where ld.loan_no = ldi.loan_no) as total_loan_due,
	(select ldi.due_amt_future
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as future_due_amt,
	(select ldi.due_date
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as future_due_date,
	(select ldi.penalty_amt
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as total_penalty_due,
	(select ldi.to_close
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as amt_to_close,
	(select ldi.last_principal_paid_amt
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as last_principal_paid_amt,
	(select ldi.last_principal_paid_date
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as last_principal_paid_date,
	(select ldi.last_int_paid_amt
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as last_int_paid_amt,
	(select ldi.last_int_paid_date
	from webloan_reports.dbo.loan_data_info ldi
	where ld.loan_no = ldi.loan_no) as last_int_paid_date,
